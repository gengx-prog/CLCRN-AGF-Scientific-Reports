...
class Supervisor:
    def __init__(self, **kwargs):
        ...
        self._loss_name = self._train_kwargs.get('loss_name', 'mae')
        self._loss_mix_mse_weight = float(self._train_kwargs.get('loss_mix_mse_weight', 0.0))
        ...
        if self._model_name == 'CLCRN':
            from model.clcnn.recurrent.seq2seq_model import CLCRNModel
            model = CLCRNModel(
                self.location_info, 
                self.sparse_idx, 
                self.geodesic, 
            ...
            from model.clcnn.attention.attention_model import CLCSTNModel
            model = CLCSTNModel(
                self.location_info, 
                self.sparse_idx, 
                self.geodesic, 
            ...
            from model.baselines.recurrent.seq2seq_model import RNNModel
            model = RNNModel(
            ...
            from model.baselines.attention.attention_model import ATTModel
            model = ATTModel(
                sparse_idx=self.sparse_idx, 
        ...
        self._horizon_loss_weights = self._build_horizon_loss_weights()

    def _build_horizon_loss_weights(self):
        mode = self._train_kwargs.get('horizon_weight_mode', 'none')
        if mode in [None, 'none', 'uniform']:
            return None

        if 'horizon_loss_weights' in self._train_kwargs:
            weights = self._train_kwargs.get('horizon_loss_weights')
            if len(weights) != self.horizon:
                raise ValueError(
                    f"horizon_loss_weights length {len(weights)} does not match horizon {self.horizon}."
                )
            return torch.tensor(weights, dtype=torch.float32, device=self._device)

        start = float(self._train_kwargs.get('horizon_weight_start', 1.0))
        end = float(self._train_kwargs.get('horizon_weight_end', 1.0))
        if mode == 'linear':
            return torch.linspace(start, end, steps=self.horizon, device=self._device)
        if mode == 'exp':
            gamma = float(self._train_kwargs.get('horizon_weight_gamma', 1.0))
            indices = torch.arange(self.horizon, dtype=torch.float32, device=self._device)
            return start * torch.pow(torch.tensor(gamma, dtype=torch.float32, device=self._device), indices)
    ...
    def evaluate(self, dataset, batches_seen, epoch_num, load_model=False, steps=None):
        ...
        with torch.no_grad():
            ...
            for _, (x, y) in enumerate(val_iterator):
                x, y = self._prepare_data(x, y)
                output = self.model(x)
                loss, y_true, y_pred = self._compute_loss(y, output, use_training_loss=False)
            ...
            # === NMI Experiment A2: per-node MAE ===
            pred_np  = y_preds.numpy()   # (H, total_samples, N, out_dim)
            truth_np = y_truths.numpy()
            per_node_mae = np.mean(np.abs(pred_np - truth_np), axis=(0, 1, 3))  # (N,)
            npy_path = os.path.join(self._log_dir, 'per_node_mae.npy')
            np.save(npy_path, per_node_mae)
            self._logger.info(f'Per-node MAE saved → {npy_path}')
            # === end NMI A2 ===
    ...
    def _train(self, base_lr,
        ...
        use_amp = bool(kwargs.get('use_amp', False)) and self._device.type == 'cuda'
        scaler = torch.amp.GradScaler('cuda', enabled=use_amp)
        if use_amp:
            self._logger.info('Using CUDA automatic mixed precision for training.')
        ...
        for epo in range(self._epoch_num, epochs):
            ...
            for _, (x, y) in enumerate(progress_bar): 
                ...
                if use_amp:
                    with torch.amp.autocast('cuda', dtype=torch.float16):
                        ...
                        loss, y_true, y_pred = self._compute_loss(y, output, use_training_loss=True)
                else:
                    output = self.model(x, y, batches_seen = batches_seen)
                    loss, y_true, y_pred = self._compute_loss(y, output, use_training_loss=True)
            ...
            if (epoch_num % log_every) == 0:
                train_label = 'train_mae' if self._loss_name == 'mae' else 'train_loss'
                message = 'Epoch [{}/{}] ({}) {}: {:.4f}, val_mae: {:.4f}, lr: {:.6f}, ' \
                          '{:.1f}s'.format(epoch_num, epochs, batches_seen,
                                           train_label, np.mean(losses), val_loss, lr_scheduler.get_last_lr()[0],
                                           (end_time - start_time))
            ...
            if (epoch_num % test_every_n_epochs) == 0:
                ...
                train_label = 'train_mae' if self._loss_name == 'mae' else 'train_loss'
                message = 'Epoch [{}/{}] ({}) {}: {:.4f}, test_mae: {:.4f},  lr: {:.6f}, ' \
                          '{:.1f}s'.format(epoch_num, epochs, batches_seen,
                                           train_label, np.mean(losses), test_loss, lr_scheduler.get_last_lr()[0],
                                           (end_time - start_time))
                self._logger.info(message)
    ...
    def _compute_loss(self, y_true, y_predicted, use_training_loss=True):
        ...
        elif self._loss_name == 'weighted_mae':
            loss = masked_mae_loss(y_predicted, y_true, horizon_weights=self._horizon_loss_weights)
        elif self._loss_name == 'weighted_mae_mse':
            loss = masked_mae_loss(y_predicted, y_true, horizon_weights=self._horizon_loss_weights)
            if self._loss_mix_mse_weight > 0:
                loss = loss + self._loss_mix_mse_weight * masked_mse_loss(
                    y_predicted, y_true, horizon_weights=self._horizon_loss_weights
                )
        else:
